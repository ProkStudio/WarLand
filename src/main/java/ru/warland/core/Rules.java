...
public final class Rules {
    ...
    public static int claimLimit(int active,int level){return Math.min(256,9+3*Math.max(0,active)+9*Math.max(0,level));}
    public static long price(long base,int sold,int quota){if(base<=0||quota<=0)throw new IllegalArgumentException();return Math.max(1,base*Math.max(25,100-75L*Math.max(0,sold)/quota)/100);}
