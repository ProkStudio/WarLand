...
public final class CoreRuntime implements WarLandApi {
 ...
 private void onJoin(ServerPlayerEntity p){
  ...
  }).whenComplete((profile,e)->server.execute(()->{if(!online(p))return;if(e!=null){p.networkHandler.disconnect(Text.literal("Не удалось безопасно загрузить профиль."));WarLand.LOG.error("Profile initialization failed",e);return;}
 ...
 private void tick(MinecraftServer s){if(!ready)return;tick++;long now=System.currentTimeMillis();
  ...
  var iterator=teleports.entrySet().iterator();while(iterator.hasNext()){
   ...
   if(p==null||!p.isAlive()||p.hasVehicle()||inCombat(e.getKey())||inventoryLocked(e.getKey())||!canWarp(p,t.id)||!Objects.equals(warps.get(t.id),t.target)||p.getBlockPos().getSquaredDistance(t.origin)>0.5){if(p!=null)reply(p,"Перемещение отменено.");iterator.remove();continue;}
 ...
 public boolean canWarp(ServerPlayerEntity p,String id){Warp warp=warps.get(id);if(!ready||warp==null||warp.world.getRegistryKey()==net.minecraft.world.World.END)return false;var member=nations.member(p.getUuid());String claim=nations.claim(warp.world.getRegistryKey().getValue().toString(),warp.feet.getX()>>4,warp.feet.getZ()>>4);return CoreSafety.warpAllowed(id,member==null?null:member.nation(),claim);}
 ...
 @Override public boolean inventoryLocked(UUID player){return InventoryLocks.busy(player);}
 @Override public boolean canBuild(ServerPlayerEntity p,ServerWorld w,BlockPos pos){if(!ready||inventoryLocked(p.getUuid())||p.isSpectator()||w.getRegistryKey()==net.minecraft.world.World.END)return false;if(safe(w,pos))return false;String n=nations.claim(w.getRegistryKey().getValue().toString(),pos.getX()>>4,pos.getZ()>>4);return n==null||nations.allowed(p.getUuid(),n,"build");}
 ...
 @Override public void menu(ServerPlayerEntity p,String title,List<MenuEntry> entries){if(ready&&online(p)){if(inventoryLocked(p.getUuid())){reply(p,"Дождитесь завершения операции с инвентарём.");return;}Menus.open(p,title,entries);}}
