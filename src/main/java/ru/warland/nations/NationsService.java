...
public final class NationsService {
 ...
 public NationsService(Store db,GameConfig config){this.db=db;this.config=config;}
 ...
 }).thenAccept(s->cache=s);}
 private CompletableFuture<String> mutate(Store.Work<String> w){return db.tx(w).thenCompose(result->refresh().thenApply(v->result));}
 ...
 public CompletableFuture<String> claim(UUID p,String d,int x,int z){String op=UUID.randomUUID().toString();return mutate(c->{
