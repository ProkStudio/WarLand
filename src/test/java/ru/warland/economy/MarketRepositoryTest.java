...
class MarketRepositoryTest {
    ...
    UUID deposit(UUID owner) throws Exception {
        ...
        market.prepareDeposit(id, owner, STACK, "before", "after", NOW).get();
        market.reconcile(id, owner, "after", NOW).get(); return id;
    }
    UUID offer(long price, int fee) throws Exception {
        UUID id = UUID.randomUUID(); market.list(id, seller, deposit(seller), price, fee, END, NOW).get(); return id;
    }
    ...
    @Test void depositMustBeDurablyReconciledBeforeListing() throws Exception {
        UUID id = UUID.randomUUID(); market.prepareDeposit(id, seller, STACK, "before", "after", NOW).get();
        assertTrue(market.inventoryBlocked(seller).get()); assertEquals(0, count("market_items"));
        reject(market.list(UUID.randomUUID(), seller, id, 100, 0, END, NOW));
    ...
    @Test void unknownInventoryLocksUntilEvidenceResolves() throws Exception {
        ...
        reject(market.prepareDeposit(UUID.randomUUID(), seller, STACK, "a", "b", NOW));
    ...
    @Test void identicalDepositReplayAndReconciliationAreIdempotent() throws Exception {
        UUID id = UUID.randomUUID();
        var first = market.prepareDeposit(id, seller, STACK, "before", "after", NOW).get();
        assertEquals(first, market.prepareDeposit(id, seller, STACK, "before", "after", NOW + 1).get());
    ...
    @Test void conflictingDepositReplayFails() throws Exception {
        UUID id = UUID.randomUUID(); market.prepareDeposit(id, seller, STACK, "a", "b", NOW).get();
        reject(market.prepareDeposit(id, buyer, STACK, "a", "b", NOW));
        reject(market.prepareDeposit(id, seller, "other-stack", "a", "b", NOW));
        reject(market.prepareDeposit(id, seller, STACK, "a", "c", NOW));
        reject(market.reconcile(id, buyer, "b", NOW));
    }
    @Test void rejectsInvalidOrUnboundedSnapshotPayloads() {
        reject(market.prepareDeposit(UUID.randomUUID(), seller, STACK, "same", "same", NOW));
        reject(market.prepareDeposit(UUID.randomUUID(), seller, " ", "a", "b", NOW));
        reject(market.prepareDeposit(UUID.randomUUID(), seller, STACK, "a".repeat(262_145), "b", NOW));
        reject(market.prepareDeposit(UUID.randomUUID(), seller, STACK, "a", "b", 0));
    }
    @Test void pendingUniquenessIsPerPlayer() throws Exception {
        market.prepareDeposit(UUID.randomUUID(), seller, STACK, "a", "b", NOW).get();
        reject(market.prepareDeposit(UUID.randomUUID(), seller, STACK, "a", "b", NOW));
        market.prepareDeposit(UUID.randomUUID(), buyer, STACK, "a", "b", NOW).get(); assertEquals(2, count("market_intents"));
    }
    @Test void deliveryIsReservedAndCannotBeListedTwice() throws Exception {
        UUID item = deposit(seller), op = UUID.randomUUID();
        var first = market.prepareDelivery(op, seller, item, "empty", "full", NOW).get();
        assertEquals(first, market.prepareDelivery(op, seller, item, "empty", "full", NOW).get());
        assertEquals(ItemStatus.DELIVERING, market.item(item).get().status());
        assertTrue(market.mailbox(seller, 10).get().isEmpty());
        reject(market.list(UUID.randomUUID(), seller, item, 100, 0, END, NOW));
        reject(market.prepareDelivery(UUID.randomUUID(), seller, item, "empty", "full", NOW));
    ...
    @Test void failedDeliveryReturnsExactlyOneItemToMailbox() throws Exception {
        UUID item = deposit(seller), op = UUID.randomUUID();
        market.prepareDelivery(op, seller, item, "empty", "full", NOW).get();
        market.reconcile(op, seller, "empty", NOW).get();
        assertEquals(1, market.mailbox(seller, 10).get().size());
        assertEquals(IntentStatus.ABORTED, market.prepareDelivery(op, seller, item, "empty", "full", NOW).get().status());
        assertEquals(ItemStatus.HELD, market.item(item).get().status());
        market.prepareDelivery(UUID.randomUUID(), seller, item, "empty", "full", NOW).get();
    }
    @Test void ambiguousDeliveryKeepsReservation() throws Exception {
        UUID item = deposit(seller), op = UUID.randomUUID(); market.prepareDelivery(op, seller, item, "a", "b", NOW).get();
    ...
    @Test void cannotDeliverSomeoneElsesItemOrReuseDepositAsDelivery() throws Exception {
        ...
        reject(market.prepareDelivery(UUID.randomUUID(), buyer, item, "a", "b", NOW));
        reject(market.prepareDelivery(item, seller, item, "before", "after", NOW));
        assertEquals(ItemStatus.HELD, market.item(item).get().status());
    }
    @Test void saleAtomicallyTransfersOwnershipAndConservesFunds() throws Exception {
        UUID listing = offer(101, 250); fund(buyer, 200);
        Trade trade = market.buy(UUID.randomUUID(), buyer, listing, 101, NOW).get();
    ...
    @Test void purchaseReplayAfterExpiryDoesNotPayTwice() throws Exception {
        ...
        var receipt = market.buy(op, buyer, listing, 100, NOW).get();
        assertEquals(receipt, market.buy(op, buyer, listing, 100, END + 1).get());
        assertEquals(100, store.balance(buyer).get()); assertEquals(1, count("market_trades"));
        reject(market.buy(op, buyer, listing, 99, NOW)); reject(market.buy(op, seller, listing, 100, NOW));
        reject(market.buy(op, buyer, UUID.randomUUID(), 100, NOW));
    }
    @Test void competingBuyersHaveExactlyOneWinner() throws Exception {
        ...
        for (UUID id : buyers) attempts.add(market.buy(UUID.randomUUID(), id, listing, 100, NOW).handle((v, e) -> e == null));
    ...
    @Test void insufficientFundsRollsBackEverything() throws Exception {
        ...
        reject(market.buy(UUID.randomUUID(), buyer, listing, 100, NOW));
    ...
    @Test void orphanLedgerReceiptCannotMakeAnUnpaidPurchase() throws Exception {
        ...
        reject(market.buy(op, buyer, listing, 100, NOW)); assertEquals(ListingStatus.OPEN, market.listing(listing).get().status());
    }
    @Test void selfPurchaseWrongPriceAndExactExpiryAreRejected() throws Exception {
        ...
        reject(market.buy(UUID.randomUUID(), seller, listing, 100, NOW));
        reject(market.buy(UUID.randomUUID(), buyer, listing, 99, NOW));
        reject(market.buy(UUID.randomUUID(), buyer, listing, 100, END));
    ...
    @Test void maximumPriceAndFullFeeDoNotOverflow() throws Exception {
        ...
        assertEquals(MAX_MONEY, market.buy(UUID.randomUUID(), buyer, listing, MAX_MONEY, NOW).get().fee());
    ...
    @Test void listingReplayIsBoundToAllTermsAndSurvivesExpiry() throws Exception {
        ...
        var first = market.list(listing, seller, item, 100, 500, END, NOW).get();
        assertEquals(first, market.list(listing, seller, item, 100, 500, END, END + 1).get());
        reject(market.list(listing, seller, item, 101, 500, END, NOW));
        reject(market.list(listing, seller, item, 100, 501, END, NOW));
        reject(market.list(listing, seller, item, 100, 500, END + 1, NOW));
        reject(market.list(listing, buyer, item, 100, 500, END, NOW));
        reject(market.list(UUID.randomUUID(), seller, item, 100, 500, END, NOW));
    }
    @Test void cancelledItemCanBeRelistedButOldListingCannotBePurchased() throws Exception {
        ...
        reject(market.cancel(listing, buyer, NOW));
        assertEquals(ListingStatus.CANCELLED, market.cancel(listing, seller, NOW).get().status());
        market.cancel(listing, seller, NOW).get(); assertEquals(1, market.mailbox(seller, 10).get().size());
        market.list(UUID.randomUUID(), seller, item, 100, 0, END, NOW).get(); fund(buyer, 100);
        reject(market.buy(UUID.randomUUID(), buyer, listing, 100, NOW));
    }
    @Test void soldListingCannotBeCancelledByFormerOwner() throws Exception {
        UUID listing = offer(100, 0); fund(buyer, 100); market.buy(UUID.randomUUID(), buyer, listing, 100, NOW).get();
        reject(market.cancel(listing, seller, NOW));
    ...
    @Test void cancelAtExpiryUsesExpiredState() throws Exception {
        UUID listing = offer(100, 0); assertEquals(ListingStatus.EXPIRED, market.cancel(listing, seller, END).get().status());
    }
    @Test void boundsRejectInvalidPricesFeesAndPageSizes() throws Exception {
        UUID item = deposit(seller);
        for (long price : new long[] {0, -1, MAX_MONEY + 1}) reject(market.list(UUID.randomUUID(), seller, item, price, 0, END, NOW));
        for (int fee : new int[] {-1, 10_001}) reject(market.list(UUID.randomUUID(), seller, item, 100, fee, END, NOW));
        reject(market.list(UUID.randomUUID(), seller, item, 100, 0, NOW, NOW));
    ...
    @Test void pendingBuyerCannotStartPurchase() throws Exception {
        ...
        market.prepareDeposit(UUID.randomUUID(), buyer, STACK, "a", "b", NOW).get();
        reject(market.buy(UUID.randomUUID(), buyer, listing, 100, NOW)); assertEquals(100, store.balance(buyer).get());
    }
    ...
    @Test void catalogueIsBoundedAndExcludesExpiredAndSoldListings() throws Exception {
        ...
        fund(buyer, 100); market.buy(UUID.randomUUID(), buyer, sold, 100, NOW).get();
